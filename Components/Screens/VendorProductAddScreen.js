...
export default function VendorProductAdd({ navigation, route }) {
    ...
    if (!result.canceled) {
      if (forw === "images") {
        ...
        if (valueProductType === "single") {
          ...
          // console.log(JSON.stringify(altValue));
          setImgIndicator2(false);
        }
      }
    }
  ...
  const submitProduct = async () => {
    ...
    const product = {
      bannerImage,
      categorie: valueCategorieType.cat,
    ...
    // console.log(JSON.stringify(product.categorie));
    const headers = {
      jwt: await SecureStore.getItemAsync("jwt"),
    };
    console.log(product);
